    ...
    <div className="inline-flex flex-col items-start gap-1">
      <button
        ...
        className={`inline-flex items-center gap-1 rounded-full px-3 py-1 text-sm font-bold transition disabled:cursor-not-allowed disabled:opacity-50 ${
          isDark ? 'bg-white/20 text-white hover:bg-white/30' : 'bg-indigo-50 dark:bg-indigo-950/50 text-indigo-700 hover:bg-indigo-100'
        }`}
        aria-label={label}
        title={label}
      >
      ...
      {status && <span className={`max-w-56 text-xs ${isDark ? 'text-white/80' : 'text-slate-500 dark:text-slate-400'}`}>{status}</span>}
    </div>
  );
}
  ...
  return (
    <div role="button" tabIndex={0} onClick={onFlip} onKeyDown={handleKeyDown} className="card-3d group h-[30rem] w-full cursor-pointer text-left outline-none focus-visible:ring-4 focus-visible:ring-indigo-200" aria-label={flipped ? '단어 카드 앞면 보기' : '단어 카드 뜻 보기'}>
      <div className={`card-inner relative h-full w-full rounded-[2rem] transition-transform duration-500 ${flipped ? '[transform:rotateY(180deg)]' : ''}`}>
        <section className="card-face absolute inset-0 flex h-full w-full flex-col justify-between rounded-[2rem] border border-indigo-100 dark:border-slate-800 bg-white dark:bg-slate-900 p-8 shadow-xl shadow-indigo-100/60">
          <div className="flex items-center justify-between gap-3">
            <span className="rounded-full bg-indigo-50 dark:bg-indigo-950/50 px-3 py-1 text-sm font-semibold text-indigo-600">{languageLabel}</span>
            <span className="text-sm text-slate-500 dark:text-slate-500">클릭해서 뜻 보기</span>
          </div>
          <div>
            <p className="text-sm font-medium uppercase tracking-[0.35em] text-slate-500 dark:text-slate-500">WORD</p>
            <div className="mt-4 flex flex-wrap items-center gap-3">
              <h2 className="break-words text-5xl font-black tracking-tight text-slate-950 dark:text-white sm:text-6xl">{word.word}</h2>
              <SpeakButton text={word.word} lang={speechLang} label="단어 듣기" />
            </div>
            {word.reading && <p className="mt-4 text-xl text-slate-500 dark:text-slate-400">{word.reading}</p>}
            {androidEdgeWarning && <p className="mt-4 rounded-2xl bg-amber-50 dark:bg-amber-950/40 p-3 text-xs leading-5 text-amber-700 dark:text-amber-300">기기 음성을 먼저 재생합니다. 지원되지 않으면 서버 음성으로 전환합니다.</p>}
          </div>
          <p className="text-sm text-slate-500 dark:text-slate-500">학습모드는 뜻과 예문 해석을 볼 수 있습니다</p>
        </section>
